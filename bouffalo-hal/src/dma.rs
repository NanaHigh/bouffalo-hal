...
impl ChannelConfig {
    ...
    /// Set destination peripheral for DMA0/1.
    #[inline]
    ...
    /// Get destination peripheral for DMA0/1.
    #[inline]
    ...
    /// Set source peripheral for DMA0/1.
    #[inline]
    ...
    /// Get source peripheral for DMA0/1.
    #[inline]
...
impl<DMA: Deref<Target = RegisterBlock>> Dma<DMA> {
    #[inline]
    pub fn test(&self, src_addr: u32) {
        unsafe {
            self.dma.global_config.modify(|val| {
                val.set_ahb_master_endian_mode(EndianMode::LittleEndian)
                    .enable_smdma()
            });
            self.dma.channels[self.ch as usize].control.modify(|val| {
                val.enable_cplt_int()
                    .enable_src_addr_inc()
            ...
            self.dma.channels[self.ch as usize]
                .source_address
                .write(src_addr);
            self.dma.channels[self.ch as usize]
                .destination_address
                .write(0x2000A088);
            self.dma.channels[self.ch as usize].config.modify(|val| {
                val.set_dma_mode(DMAMode::Mem2Periph)
                    // .set_dst_periph4dma01(Periph4DMA01::Uart0Tx)
                    .set_src_periph4dma01(Periph4DMA01::Uart0Rx)
                    .enable_ch()
            });
            self.dma.interrupts.transfer_complete_clear.write(TransferCompleteClear(0x0).clear_cplt_int(self.ch as u8));
            self.dma.interrupts.error_clear.write(ErrorClear(0x0).clear_err_int(self.ch as u8));
        }
    }

    #[inline]
    pub fn new(dma: DMA, ch: u8) -> Self {
        Self { dma, ch }
    }
}
